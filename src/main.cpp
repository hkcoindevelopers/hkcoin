...
#if defined(NDEBUG)
# error "HKcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb109c8236b24b1c922b395f287ea35282dfe388f8404dd9faabf47852fbb87d6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HKcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "HKcoin Signed Message:\n";
...
{
    // HKcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // HKcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // HKcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // HKcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // HKcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // HKcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x612444e75035db16929d9d0e39491ae05af6240c98e7b455e950620d51589eb5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04145ca1c3d3a6f679518b05d443a08116277a25f1be48ff7eb1b5edef7d7ea61e6594e57806a5656e628be52a04c8e65926305ef7474254a3eb730589f1ccba7d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515926823;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086065352;

        if (fTestNet)
        {
            block.nTime    = 1515926755;
            block.nNonce   = 386411458;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x82cff4b46082b8e978b496ca75ddfbf2e213f4e04cf45752b17dde113b84abe7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc9, 0xb8, 0xd7 }; // HKcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HKcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HKcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HKcoinMiner(CWallet *pwallet)
{
    printf("HKcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hkcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HKcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HKcoinMiner, pwallet));
}
